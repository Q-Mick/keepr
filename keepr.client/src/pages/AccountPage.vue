<template>
    ...
    <div class="w-full px-0 sm:px10 sm:w-[80%]">
      <div class="flex flex-col column-3 justify-center items-center align-middle mt-7">
        ...
        <div class="w-full px-5 sm:px-0 sm:max-w-[50rem] h-96">
          <div class="relative h-full w-full">
            <img class="h-full w-full object-fit aspect-video sm:object-cover" :src="account.coverImg" alt="Banner"
              title="Account cover image">
            <div
              class="avatar absolute bottom-[-2rem] left-[39%] sm:left-[45%] rounded-full border border-white elevation">
      ...
      <div v-if="vaults.length != 0">
        <div class="relative grid grid-cols-2 sm:grid-cols-4 gap-x-5 gap-y-1">
          <Vault v-for="vault in vaults" :key="vault.id" :vault="vault" class="my-2 border min-h-full border-zinc-600">{{
            vault.name }}</Vault>
      ...
      <div v-if="keeps.length != 0">
        <div class="relative columns-2 sm:columns-4 gap-3 px-1 sm:pr-0 sm:gap-8 pt-5 mb-5">
          <Keep @click="setActiveKeep(keep.id)" v-for="keep in keeps" :key="keep.id" :keep="keep"
            :show-creator-pic="false" class="my-5 border border-zinc-600"
  ...
  <KeepDetails @toggle-details="closeModal" v-model:vault="myVaults" v-model:isOpen="openDetails"
    v-model:keep="actKeep" />
